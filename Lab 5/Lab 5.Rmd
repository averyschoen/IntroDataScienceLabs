...
1.  Our ultimate goal in this lab is to predict the number of hours a woman works outside the home. Make a histogram for `hours`, the total number of hours a woman works outside the home in a year.
...
Identify categorical variables and convert them to indicators.
...
9.  Now use `.predict()` to get the predicted values for the test set (save them as `preds_ols`).
...
```{python ridge, exercise = TRUE, message = FALSE, exercise.setup="setup6"}
...
11. In the case of `.Ridge()`, we will be using `alpha` where `alpha = 0` is equivalent to least squares regression and large values of `alpha` lead to more regularization. Adapt the code below to change the value of `alpha`, just to test the function.
...
```{python setup71, exercise = TRUE, message = FALSE, exercise.setup="setup7"}
model = sklearn.linear_model.Ridge(alpha = 0.5)
modelRidge = model.fit(X_train_pp, y_train)
```

12. Now, we are ready to use cross validation to try and select the best `alpha` for our dataset. Usually, we are looking over a range of numbers--let's go ahead and create one now using `np.arange()` (`range()` is just for floats). Edit the code below to create a range of numbers to use for `alpha` from -6 to 2 in increments of 0.1.
...
```{python alpha, exercise = TRUE, message = FALSE, exercise.setup="setup7"}
alphas = ...
...
13. For the actual selection process, we could write some code from scratch, but let's also consider yet another method built into `sklearn`--[sklearn.linear_model.RidgeCV](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.RidgeCV.html#sklearn.linear_model.RidgeCV).
...
```{python cv, exercise = TRUE, message = FALSE, exercise.setup="setup8"}
...
14. Take a closer look at the documentation for the `cv` argument--you'll see there are actually options for all three CV methods we have learned so far:
...
15. Note that the code in 14. produces only one set of coefficients with the chosen `alpha`--so we know what the threshold is, but we don't have the nice plots we are used to seeing. To do that, we need to write a loop fitting the model multiple times, saving the following:
...
16. Now we can start creating the loop. Let's work on the calculations we would like to repeat first. Code one instance of the loop below, using code you wrote in Steps 13 and 15 but taking steps to make it more general. Some tips:
...
17. Now, make your code from 17 into a loop by adding a `for` statement. I found it most helpful to loop over a range of integers because of `.loc[]`.
...
19. Now let's work on making the plot comparing the coefficients to the threshold. You will want to put the `alphas` on the $x$-axis, and the coefficients on the $y$-axis. You have used `geom_histogram()` and `geom_point()` before, but let's introduce a new geom, `geom_line()`. This will plot all values and connect them with a line. Code the plot below. 
...
21. Now, we can use `geom_line()` to make the plot we want with `coefs_ridge_melt`. Add an argument for the color so you get nice separation.
...
22. Most of the plots we saw in class are "flipped"--the large values of the coefficients are on the righthand side of the graph. To make this switch, you can add another command to your graph, `scale_x_reverse()`.
...
25. To compare the models we will calculate the sum of squared errors for the from each model. Adapt the code used below that calculates the sum of squared errors for linear regression and apply it to the ridge regression and LASSO models.
