...
question("4. Predict the class for point J, based on its three nearest neighbors.",
         answer("Red (class 1)"),
         answer("Blue (class 2)", correct=TRUE, message="Point H is Class 1 (red circles), but points A and B are Class 2 (blue triangles). Since 2/3 points are Class 2, we would predict point J to also be Class 2."))
```
...
\begin{align*}
\color{black}{Accuracy} & \color{black}{ = \frac{TP + TN}{n}} \\
\end{align*}

```{r q9, echo=FALSE}
question("Based off of your confusion matrix, what is the training set accuracy of the classifier?",
         answer("4/13"),
         answer("4/5"),
         answer("9/13"),
         answer("7/9"),
...
\begin{align*}
\color{black}{Sensitivity} & \color{black}{ = \frac{TP + FN}{n}} \\
& \color{black}{ = \frac{5 + 2}{13}} \\
\end{align*}
...
13. Using the correct command from Step 1., write a line of code that specifies the number of neighbors in your classifier as $k = 5$. No need to fit anything yet, but name the model `knn5`. 
...
14. Now, fit your classifier from Step 2. with `X_pp` and `Y` from Step 1.
...
fpr, tpr, thresholds = metrics.roc_curve(birthwt['low'], knn5_preds_proba[:,1], pos_label=1)
print(metrics.auc(fpr, tpr))
```
...
```{r chooose-code-check, echo=FALSE, message = FALSE, warning = FALSE}
grade_this_code()
```
...
Now, run the code below to create a plot showing the relationship between $k$ and test set accuracy.
